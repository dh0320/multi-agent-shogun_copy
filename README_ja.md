...
./start.sh          # 推奨: 統一エントリーポイント
# または: ./shutsujin_departure.sh  # 従来のスクリプト
```

エイリアスを設定済みなら、`css` と入力するだけでOK
...
<details>
<summary>🌍 <b>クロスプラットフォーム対応</b>（クリックで展開）</summary>

### 対応OS

multi-agent-shogunは以下のOSをサポートしています：
...
# インストール（全プラットフォーム共通）
./install.sh              # フルインストール
./install.sh --check      # 前提条件チェックのみ
./install.sh --deps-only  # 依存関係のみインストール

# 起動（全プラットフォーム共通）
./start.sh                # 全エージェント起動
./start.sh -s             # セットアップのみ（Claude起動なし）
./start.sh -t             # ターミナルタブを開く
...
以下の環境変数で動作をカスタマイズ可能：
- `SHOGUN_HOME`: ホームディレクトリ（自動検出）
- `SHOGUN_LANG`: 言語設定（ja, en, etc.）
- `SHOGUN_SCREENSHOT_DIR`: スクリーンショット保存先

### 抽象化レイヤー

システムはクロスプラットフォーム互換性のため、3層アーキテクチャを採用：

```
lib/
├── detect_os.sh        # OS検出と環境設定
├── utils.sh            # 共通ユーティリティ関数
└── generate_config.sh  # テンプレートベースの設定生成

config/
├── settings.yaml.template  # 環境変数対応テンプレート
└── settings.yaml           # 生成された設定（gitignore対象）
```
...
| スクリプト | 用途 | 実行タイミング |
|-----------|------|---------------|
| `install.sh` | **推奨**: クロスプラットフォームインストーラー | 初回のみ |
| `start.sh` | **推奨**: 全プラットフォーム統一エントリーポイント | 毎日 |
...
| `Makefile` | 統一インターフェース: `make install`, `make start`, `make stop` | いつでも |

### `install.sh` が行うこと（クロスプラットフォーム）：
- ✅ OS検出（macOS, Linux, WSL2, Windows）
- ✅ 依存関係のインストール（tmux, Node.js, Claude Code CLI）
- ✅ テンプレートから設定ファイルを生成
- ✅ 必要なディレクトリとキューファイルを作成
- ✅ シェルエイリアスを設定

### `start.sh` が行うこと：
- ✅ OS抽象化レイヤーを読み込み
- ✅ 前提条件をチェック
- ✅ 必要に応じてsettings.yamlを生成
- ✅ shutsujin_departure.shに委譲
...
│
│  ┌─────────────────── セットアップスクリプト ───────────────────┐
├── start.sh                  # 統一エントリーポイント（推奨）
├── install.sh                # クロスプラットフォームインストーラー（推奨）
├── Makefile                  # 統一インターフェース（make start/stop/install）
├── install.bat               # Windows: 初回セットアップ
├── first_setup.sh            # Ubuntu/Mac: 初回セットアップ（レガシー）
├── shutsujin_departure.sh    # 毎日の起動（指示書自動読み込み）
│  └────────────────────────────────────────────────────────────┘
│
├── lib/                      # クロスプラットフォーム抽象化レイヤー
│   ├── detect_os.sh          # OS検出と環境設定
│   ├── utils.sh              # 共通ユーティリティ関数
│   └── generate_config.sh    # テンプレートベースの設定生成
│

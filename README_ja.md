...
| `first_setup.sh` | tmux、Node.js、AI CLI（Claude/Copilot）をインストール | 初回のみ |
| `shutsujin_departure.sh` | tmuxセッション作成 + AI CLI起動 + 指示書読み込み | 毎日 |

### `install.bat` が自動で行うこと：
- ✅ WSL2がインストールされているかチェック
- ✅ Ubuntuを開いて `first_setup.sh` を実行
- ✅ tmux、Node.js、AI CLI をインストール
- ✅ 必要なディレクトリを作成

### `shutsujin_departure.sh` が行うこと：
- ✅ tmuxセッションを作成（shogun + multiagent）
- ✅ 全10エージェントでAI CLI（Claude Code または GitHub Copilot）を起動
- ✅ 各エージェントに指示書を自動読み込み
...
| **GitHub Copilot CLI** | GitHubネイティブ統合、Plan Mode | GitHub連携が必要なタスク |
...
cli:
  ...
  agents:
    shogun:
      type: claude      # 将軍はClaude（Opus）
      model: opus
      env:
        MAX_THINKING_TOKENS: "0"
    
    ashigaru1:
      type: copilot     # 足軽1だけCopilot CLIを試す
    
    # 指定しない場合は default が適用される
```

### 使用例

#### 例1: 全エージェントでClaude Code（デフォルト）
```yaml
cli:
  default: claude
```

#### 例2: GitHub連携タスクで一部Copilotを使用
```yaml
cli:
  default: claude
  agents:
    ashigaru1:
      type: copilot  # GitHub関連のタスク専用
    ashigaru2:
      type: copilot
```
...
cli:
  ...
  agents:
    shogun:
      type: claude  # 将軍だけClaude（Opus）を使いたい
      model: opus
```
...
# Copilot CLI は GitHub CLI に統合されています
copilot --version
```
...
# GitHub Copilot CLI で強制起動（設定ファイルを無視）
./shutsujin_departure.sh --copilot

# Claude Code CLI で強制起動（設定ファイルを無視）
...
# オプションの組み合わせも可能
./shutsujin_departure.sh --copilot --setup-only
...
**CLI選択のオプション（便利！）:**
- `--copilot`: 全エージェントでGitHub Copilot CLIを使用
- `--claude`: 全エージェントでClaude Code CLIを使用
- オプションなし: `config/settings.yaml` の設定に従う

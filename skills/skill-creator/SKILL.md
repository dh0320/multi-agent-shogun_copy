---
name: skill-creator
description: 汎用的な作業パターンを発見した際に、再利用可能なCodexスキル（SKILL.md）を生成する。繰り返し使えるワークフロー、ベストプラクティス、ドメイン知識をスキル化する時に使用。
---
...
5. 保存
   - パス（推奨）: ~/.codex/skills/{skill-name}/
     - `CODEX_HOME` を使う場合: $CODEX_HOME/skills/{skill-name}/
     - `~/.codex/skills/.system/` はCodex同梱スキル領域なので触らない
   - 既存スキルと名前が被らないか確認（特に `.system` や既存スキル名）
